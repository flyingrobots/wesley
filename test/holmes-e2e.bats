...
@test "holmes investigation/verification/prediction produce artifacts" {
    ...
    run node -e "const fs=require('fs'); const data=JSON.parse(fs.readFileSync('holmes.json','utf8')); if(!data.metadata || !data.metadata.verificationStatus) throw new Error('missing metadata'); if(typeof data.metadata.verificationCount !== 'number') throw new Error('verificationCount missing'); if(!data.metadata.bundleVersion) throw new Error('missing bundle version'); if(!data.scores) throw new Error('missing scores'); ['scs','tci','mri'].forEach(k=>{ if(typeof data.scores[k] !== 'number') throw new Error('missing scores.'+k); }); if(!data.breakdown || !data.breakdown.scs || !data.breakdown.tci || !data.breakdown.mri) throw new Error('missing breakdown'); if(typeof data.breakdown.scs.sql.score !== 'number' && data.breakdown.scs.sql.score !== null) throw new Error('missing scs sql score'); if(!Array.isArray(data.evidence)) throw new Error('evidence array missing'); if(!data.verdict || !data.verdict.code) throw new Error('missing verdict');"
    assert_success
