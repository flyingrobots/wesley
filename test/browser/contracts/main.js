...
async function verifyIr() {
  const { createBrowserRuntime } =
    await import('../../../packages/wesley-host-browser/src/createBrowserRuntime.mjs');
...
function createStatusIcon(ok) {
  ...
  title.textContent = ok ? 'pass' : 'fail';
  ...
  path.setAttribute(
    'd',
    ok
      ? 'M9 16.2l-3.5-3.5L4 14.2l5 5 11-11-1.5-1.5z'
      : 'M18.3 5.71L12 12.01 5.7 5.71 4.29 7.12 10.59 13.4 4.29 19.7l1.41 1.41 6.3-6.3 6.29 6.3 1.41-1.41-6.29-6.3 6.29-6.28z'
  );
...
function createReportHeader(res) {
  ...
  h1.appendChild(document.createTextNode('Host Contracts (Browser)'));
  const meta = document.createElement('p');
  meta.className = CSS_CLASSES.meta;
  const stat = (label, value) => {
    const wrap = document.createElement('span');
    wrap.className = CSS_CLASSES.stat;
    const l = document.createElement('span');
    l.className = CSS_CLASSES.statLabel;
...
function createFailureDetailsElement(d) {
  ...
  details.className = CSS_CLASSES.details;
  details.open = true;
  const summary = document.createElement('summary');
  summary.className = CSS_CLASSES.summary;
  const label = document.createElement('span');
  label.appendChild(document.createTextNode('IR diagnostics'));
  ...
  if (typeof d.expectedTableCount === 'number' || typeof d.actualTableCount === 'number') {
    ...
    row.appendChild(
      document.createTextNode(
        `tables: expected=${d.expectedTableCount} actual=${d.actualTableCount}`
      )
    );
    wrap.appendChild(row);
  }
  const mt =
    Array.isArray(d.missingTables) && d.missingTables.length ? d.missingTables.join(', ') : '—';
  const rowMt = document.createElement('div');
  rowMt.appendChild(document.createTextNode(`missing tables: ${mt}`));
  wrap.appendChild(rowMt);
  const rowMc = document.createElement('div');
  rowMc.appendChild(document.createTextNode('missing columns: '));
  const mcCode = document.createElement('code');
  mcCode.className = CSS_CLASSES.code;
  mcCode.textContent =
    d.missingColumns && typeof d.missingColumns === 'object'
      ? JSON.stringify(d.missingColumns)
      : '—';
  rowMc.appendChild(mcCode);
  wrap.appendChild(rowMc);
  if (d.summary) {
    const row = document.createElement('div');
    row.appendChild(document.createTextNode(`summary: ${String(d.summary)}`));
    wrap.appendChild(row);
  }
  if (d.sdlSnippet) {
    const pre = document.createElement('pre');
    pre.className = CSS_CLASSES.pre;
    pre.textContent = String(d.sdlSnippet);
...
function createTestCaseElement(c) {
  ...
  badge.className = c.ok ? CSS_CLASSES.badgeOk : CSS_CLASSES.badgeFail;
  badge.appendChild(document.createTextNode(c.ok ? 'PASS' : 'FAIL'));
  li.appendChild(badge);
  const code = document.createElement('code');
  code.className = CSS_CLASSES.code;
