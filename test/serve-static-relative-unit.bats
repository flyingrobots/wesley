...
@test "isWithinRoot() true for inside, false for outside" {
  ...
  run bash -lc "ROOT='$ROOT' IN='$INSIDE' OUT='$OUTSIDE' node -e \"import('${PWD//\//\/}/scripts/serve-static.mjs').then(m=>{console.log(m.isPathWithinRoot(process.env.ROOT, process.env.IN)); console.log(m.isPathWithinRoot(process.env.ROOT, process.env.OUT));})\""
  assert_success
