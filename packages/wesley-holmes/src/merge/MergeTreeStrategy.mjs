...
export class MergeTreeStrategy {
  ...
  execute(plan) {
    ...
    try {
      execFileSync(
        'git',
        ['fetch', '--prune', 'origin', `${baseRef}:refs/remotes/origin/${baseRef}`],
        {
          stdio: 'ignore',
          cwd: this.repoRoot
        }
    ...
    try {
      const out = execFileSync(
        'git',
        ['merge-tree', '--write-tree', mergeBase, 'HEAD', remoteBase],
        {
          encoding: 'utf8',
          cwd: this.repoRoot
        }
    ...
    try {
      baseTree = execFileSync('git', ['rev-parse', `${remoteBase}^{tree}`], {
        encoding: 'utf8',
        cwd: this.repoRoot
      }).trim();
    } catch {
