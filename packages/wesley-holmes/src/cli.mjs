...
import { inspectPersistedRuntimeRun, listPersistedRuntimeRuns } from './runtime-run.mjs';
...
import { holmesReportSchema, watsonReportSchema, validateReport } from './report-schemas.mjs';
...
function loadBundle(bundlePath) {
  ...
  throw new Error(
    `No Wesley bundle found at ${resolved}. Generate or restore a Wesley evidence bundle before running HOLMES.`
  );
}
...
async function main() {
  ...
  program
    ...
    .option(
      '--bundle-dir <path>',
      'Path to Wesley generated artifact directory',
      GENERATED_ARTIFACT_DIR
    )
    ...
    .addHelpText(
      ...
      `
Requires:
  .wesley-cache/bundle.json   Generated or restored before HOLMES runs
...
"When you have eliminated the impossible, whatever remains,
 however improbable, must be the deployable."`
  ...
  runsCommand
    ...
    .option(
      '--transmutation <name>',
      `Filter runs by transmutation name (${HOLMES_COMMAND_TRANSMUTATIONS.join(', ')})`
    )
  ...
  runsCommand
    ...
    .action(async (options) => {
      ...
      if (payload.run.failure?.code) {
        console.log(
          `Failure: ${payload.run.failure.code}${payload.run.failure.message ? ` - ${payload.run.failure.message}` : ''}`
        );
      }
    });
