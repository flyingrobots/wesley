export function renderMoriartyPrediction(data, { context = {}, minSlope = 0.01 } = {}) {
  ...
  if (data.explain) {
    ...
    if (Number.isFinite(r.ci.value)) {
      report.push(`- CI Stability ≥ ${(data.explain.thresholds.ci * 100).toFixed(0)}% (branch ${context?.ci?.branch || 'base'}) → ${r.ci.pass ? 'PASS ✅' : 'FAIL ❌'} (actual ${(r.ci.value * 100).toFixed(0)}% over ~${r.ci.windowHours ?? '?'}h)`);
    }
    if (r.evidenceTrust) {
