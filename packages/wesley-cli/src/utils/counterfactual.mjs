...
export async function maybeAnalyzeCounterfactual({
  ...
  const enabled =
    typeof options.counterfactual !== 'undefined' || Boolean(policy?.counterfactual?.enabled);
  if (!enabled) return null;

  const baseRef =
    typeof options.counterfactual === 'string' && options.counterfactual.length > 0
      ? options.counterfactual
      : policy?.counterfactual?.baseRef ||
        process.env.MORIARTY_BASE_REF ||
...
export function assertCounterfactualGate(summary) {
  ...
  const reasons = Array.isArray(summary?.judgment?.reasons)
    ? summary.judgment.reasons.join(' ')
