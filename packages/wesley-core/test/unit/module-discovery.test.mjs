...
function makeCapabilityMatrix(prefix) {
  return Object.fromEntries(
    Object.entries(WESLEY_MODULE_CAPABILITY_COLLECTIONS).map(([area, collections]) => [
      area,
      Object.fromEntries(
        collections.map((collection) => [collection, [{ name: `${prefix}-${area}-${collection}` }]])
      )
    ])
  );
}
...
test('discoverModules loads string and object entries', async () => {
  ...
  const result = await discoverModules(['alpha', { specifier: 'beta', config: { answer: 42 } }], {
    resolve,
    logger: nullLogger
  });
  ...
  assert.deepEqual(
    result.capabilityRegistry.modules.map((module) => module.name),
    ['alpha', 'beta']
  );
});
...
test('discoverModules forwards config to init()', async () => {
  ...
  await discoverModules([{ specifier: 'configured', config: { mode: 'test' } }], {
    resolve,
    logger: nullLogger
  });
...
test('discoverModules skips disabled entries', async () => {
  ...
  const result = await discoverModules([{ specifier: 'skipped', enabled: false }, 'loaded'], {
    resolve,
    logger: nullLogger
  });
...
test('createModuleCapabilityRegistry aggregates structured capabilities with ownership', () => {
  ...
  assert.deepEqual(listModuleCapabilities(registry, 'wesley', 'targets'), [
    { moduleName: 'alpha', value: { name: 'alpha-target' } },
    { moduleName: 'beta', value: { name: 'beta-target' } }
  ]);
  assert.deepEqual(listModuleCapabilities(registry, 'holmes', 'scopes'), [
    { moduleName: 'alpha', value: { name: 'alpha-scope' } }
  ]);
  assert.deepEqual(listModuleCapabilities(registry, 'blade', 'gates'), [
...
test('discoverModules rejects unresolved modules with a coded error', async () => {
  const error = await catchReject(() =>
    discoverModules(['missing'], {
      resolve: mockResolve({}),
      logger: nullLogger
    })
  );
