...
{"date":"2025-10-08","time":"07:50Z","who":"Codex","summary":"CLI bats enabled locally; ops schema + Ajv; JSON-mode fixes; exit codes corrected","commits":["(workspace)","scripts/dev/setup-bats-plugins.sh","core:qir op.schema","cli:generate ajv","cli:wcmd json+exit"],"ci_status":"local: preflight OK; CLI bats 25/25 green","highlights":["Added opJsonSchema + Ajv validation before OpPlanBuilder","Emitted tests-ops.sql and ops/*.fn.sql under example/out","Silenced directive warn by default (WESLEY_WARN_DIRECTIVES=1 to enable)","WesleyCommand: JSON no-op logger; no wrapper JSON; name-aware exit codes","CLI bats runner: setup-bats-plugins script; tests updated to --out-dir"]}
