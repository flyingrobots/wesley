...
| `check-ir-parity.mjs`        | Compares explicit JS/Rust IR parity projections over the v0 corpus with bounded Alfred process calls for native lowerer probes. | `pnpm parity:ir`                                                                                                                      |
| `check-parser-parity.mjs`    | Compares legacy JS parse/lower acceptance with Rust schema lowering over parser-sensitive v0.0.6 fixtures.                      | `pnpm parity:parser`                                                                                                                  |
...
| `measure-ir-performance.mjs` | Measures Rust CLI `schema lower` wall-clock samples with bounded Alfred process calls, optionally comparing legacy JS lowering. | `pnpm perf:ir -- --include-legacy-js --markdown --output out/rust-ir-performance-baseline.md`                                         |
