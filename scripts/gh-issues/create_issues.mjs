...
function tryDeleteProjectField(projectId, fieldId) {
  ...
  try {
    const m1 =
      'mutation($projectId:ID!,$fieldId:ID!){ deleteProjectV2Field(input:{ projectId:$projectId, fieldId:$fieldId }){ projectV2{ id } } }';
  ...
  try {
    const m2 =
      'mutation($fieldId:ID!){ deleteProjectV2Field(input:{ fieldId:$fieldId }){ projectV2{ id } } }';
...
function getProjectId({ owner, repo, projectTitle, projectId, isOrg }) {
  ...
  if (isOrg) {
    const q =
      'query($owner:String!,$title:String!){ organization(login:$owner){ projectsV2(first:50, query:$title){ nodes{ id title number } } } }';
  ...
  const q =
    'query($owner:String!,$repo:String!,$title:String!){ repository(owner:$owner,name:$repo){ projectsV2(first:50, query:$title){ nodes{ id title number } } } }';
...
function getOrCreateFields(projectId) {
  const q =
    'query($id:ID!){ node(id:$id){ ... on ProjectV2 { fields(first:100){ nodes{ id name dataType ... on ProjectV2SingleSelectField { options{ id name } } } } } } }';
  const r = ghGraphQL(q, { id: projectId });
  const fields = (r?.data?.node?.fields?.nodes || []).map((f) => ({
    id: f.id,
    name: f.name,
    dataType: f.dataType,
    options: f.options
  ...
  const nonStandardPriority = fields.filter(
    (f) => f.name === 'Priority' && f.dataType !== 'SINGLE_SELECT'
  );
  ...
  const fieldsAfter = (rAfter?.data?.node?.fields?.nodes || []).map((f) => ({
    id: f.id,
    name: f.name,
    dataType: f.dataType,
    options: f.options
  }));
  let priorityLabel = fieldsAfter.find(
    (f) => f.name === 'Priority' && f.dataType === 'SINGLE_SELECT'
  );
  if (!priorityLabel) {
    const m =
      'mutation($projectId:ID!){ createProjectV2Field(input:{ projectId:$projectId, dataType:SINGLE_SELECT, name:"Priority", options:[{name:"P0"},{name:"P1"},{name:"P2"},{name:"P3"},{name:"P4"}]}){ projectV2Field{ id name } } }';
  ...
  try {
    ...
    if (needsUpdate) {
      ...
      const mUp =
        'mutation($projectId:ID!,$fieldId:ID!,$options:[ProjectV2SingleSelectFieldOptionInput!]!){ updateProjectV2Field(input:{ projectId:$projectId, fieldId:$fieldId, name:"Priority", dataType:SINGLE_SELECT, singleSelectOptions:$options }){ projectV2Field{ id name } } }';
  ...
  if (!estimate) {
    const m =
      'mutation($projectId:ID!){ createProjectV2Field(input:{ projectId:$projectId, dataType:NUMBER, name:"Estimate (human hours)"}){ projectV2Field{ id name } } }';
    const res = ghGraphQL(m, { projectId });
    estimate = {
      id: res?.data?.createProjectV2Field?.projectV2Field?.id,
      name: 'Estimate (human hours)',
      dataType: 'NUMBER'
    };
  }
...
function createIssue(repoId, title, body) {
  const m =
    'mutation($repoId:ID!,$title:String!,$body:String!){ createIssue(input:{repositoryId:$repoId,title:$title,body:$body}){ issue{ id number url } } }';
...
function setProjectNumber(projectId, itemId, fieldId, numberValue) {
  const m =
    'mutation($projectId:ID!,$itemId:ID!,$fieldId:ID!,$value:Float!){ updateProjectV2ItemFieldValue(input:{ projectId:$projectId, itemId:$itemId, fieldId:$fieldId, value:{ number:$value }}){ projectV2Item{ id } } }';
...
function linkIssuesGraphQL(sourceId, targetId, type) {
  ...
  const m =
    'mutation($sourceId:ID!,$targetId:ID!,$type:IssueLinkType!){ createIssueLink(input:{ sourceId:$sourceId, targetId:$targetId, type:$type }){ issueLink{ id, type } } }';
  try {
...
function main() {
  ...
  const defaultLabels = (process.env.GH_LABELS || 'merge-projection,holmes,moriarty')
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  for (const item of plan) {
    const prLabel =
      ...
      (typeof item.priority === 'number'
        ? { 5: 'P0', 4: 'P1', 3: 'P2', 2: 'P3', 1: 'P4' }[item.priority] || 'P4'
        : 'P3');
    ...
    if (estimateFieldId)
      setProjectNumber(projectId, projectItemId, estimateFieldId, item.estimateHours);
