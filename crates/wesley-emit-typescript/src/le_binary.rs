...
//!
//! Produces a TypeScript module that mirrors the wire layout of the Rust
//! `echo_wasm_abi::codec` primitives byte-for-byte:
//!
...
//! `decode<Op>Vars(b) -> <Op>Vars` pair whose layout matches the Rust
//! `__echo_wesley_generated::<Op>Vars` Encode/Decode impls in
//! `echo-wesley-gen`.
...
/// to use the jedit-style default.
...
fn emit_enum(out: &mut String, type_def: &TypeDefinition) {
    ...
    for (index, value) in type_def.enum_values.iter().enumerate() {
        let _ = writeln!(out, "        case '{value}': w.writeU32Le({index}); return;");
    }
    if type_def.enum_values.is_empty() {
    ...
    let _ = writeln!(
        out,
        "export function _dec{name}(r: Reader): {name} {{"
    );
...
mod tests {
    use super::*;
    use wesley_core::{lower_schema_sdl, list_schema_operations_sdl};
