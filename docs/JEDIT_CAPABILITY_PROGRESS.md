...
- The LE binary codec intentionally emits operation variable codecs only.
  Target dispatch identifiers are no longer generated by Wesley core; Echo or
  another target extension must derive and publish those identifiers in its own
...
- In the Echo repository: consume Wesley's schema operation catalog or generated
  ...
  and dispatch identifiers in Echo rather than moving them into
  `wesley-core`.
