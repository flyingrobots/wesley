...
| Autotag                 | `.continuum/release.yml` names `.github/workflows/release-autotag.yml`. When a `release/vX.Y.Z` prep PR merges, it waits for the commit's other CI runs, runs the full release guard against a local annotated tag, and pushes that unsigned tag only if `main` is still the release commit. It never publishes or moves a tag. A signed manual tag is the fallback when autotag cannot run. |
