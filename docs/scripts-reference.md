...
| `pnpm run format` / `format:check`                              | Prettier formatting helpers.                                                                      | `format` rewrites Prettier-owned source/docs/config files, `format:check` is read-only. Wesley SDL files and Rust IR golden bytes are excluded because their bytes are compiler evidence. |
