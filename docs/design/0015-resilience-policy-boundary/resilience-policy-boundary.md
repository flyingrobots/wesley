...
3. Prove the wrapper preserves normal lowering errors and maps policy timeouts
   to Wesley resilience diagnostics.
...
- `crates/wesley-core` exposes `ResiliencePolicy` and
  `ResilientLoweringPort` for opt-in Rust lowering timeouts.
...
4. Do JS parity child processes have bounded timeout and output behavior?
5. Does the design keep resilience policy outside product and database
   ownership?
