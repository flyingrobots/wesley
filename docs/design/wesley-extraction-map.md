...
| Continuum realization verifier               | removed `verify-realization.mjs`, `realization-integrity.mjs`, root `verify:realization`, stale pre-commit and GitHub preflight invocations, `verify-realization.bats`, and compile-dependent witness assertions                                                                                                                                                              | Done                      | Wesley no longer advertises, tests, hook-runs, or CI-runs a built-in two-leg Continuum realization verifier. Recreate only as a Continuum-owned module surface.                                                                      |
...
| Echo runtime/CAS/ABI schemas                 | removed Wesley-local `schemas/echo-core-types.graphql` and `schemas/echo-wasm-abi.graphql`; Echo tracks reconciliation in its own GitHub Issues instead of committing stale SDL as active Wesley truth                                                                                                                                                                        | Done                      | Echo owns Echo-local runtime, CAS, and WASM ABI schema truth; old Wesley SDL concepts must be re-authored there only if they still match current Echo truth.                                                                         |
...
- `packages/wesley-tasks/` has been deleted; generic Wesley no longer owns a
  JavaScript DAG runtime
