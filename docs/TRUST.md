...
Wesley emits evidence artifacts (bundle, scores, SHIPME) to build trust in generated plans. Certificates include hashes and metadata so deployments are auditable, and SHIPME now summarizes citation quality so exact evidence, whole-file fallbacks, and coarse citations are visible instead of flattened into one trust signal.
