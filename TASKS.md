...
- [ ] **C1 — `Predicate.isNull()`/`isNotNull()` runtime crash**
...
- [ ] **C2 — SQL injection via unvalidated `typeHint` on `ParamRef`**
...
- [ ] **C3 — SQL injection via unvalidated `type` on `Literal` nodes**
...
- [ ] **C4 — SQL injection via unvalidated `nulls` in ORDER BY**
...
- [ ] **C5 — Sign/verify canonicalization mismatch breaks multi-signature certs**
...
- [ ] **M1 — `qir` subcommands silently drop global CLI options**
...
- [ ] **M2 — `validateRealm` can mask original error in failure path**
...
- [ ] **M3 — Unvalidated `s.type`, `s.default`, `s.using` in migration SQL emission**
...
- [ ] **M4 — `LIMIT NaN` emitted for non-numeric strings**
...
- [ ] **M5 — `Cursor.mjs` not exported from barrel file**
  `packages/wesley-core/src/domain/qir/index.mjs` is missing `export * from './Cursor.mjs';`.
  **Fix:** Add the export line.

- [ ] **M6 — `Cursor.mjs` uses `Buffer` — violates platform-agnostic constraint**
...
- [ ] **M7 — Docs reference non-existent `schemas/plan.schema.json`**
...
- [ ] **M8 — `ParamCollector` silently ignores unrecognized predicate kinds**
...
- [ ] **m1 — `this.ctx.logger` vs `logger` in `generate.mjs`**
...
- [ ] **m2 — `fs.readFile()` vs `fs.read()` inconsistency**
...
- [ ] **m3 — `process.env` / `globalThis.wesleyCtx` bypass DI context**
...
- [ ] **m4 — Duplicated migration logic between `plan.mjs` and `rehearse.mjs`**
...
- [ ] **m5 — Duplicate `sanitizeIdentBase` implementations**
...
- [ ] **m6 — `decodeCursor` swallows errors; no `__proto__` filtering**
...
- [ ] **m7 — `ir-family.md` describes envelope as "Planned (future)"**
  `docs/spec/ir-family.md:24-26` — envelope ships in this PR.
  **Fix:** Update prose to present tense.

- [ ] **m8 — `ir-family.md` references wrong bats test path**
...
- [ ] **m9 — `ir-family-spec.md` uses future tense for existing schemas**
  Lines 76, 82, 128 say "will live at" / "will complete" for schemas that already ship.
  **Fix:** Change to present tense.

- [ ] **m10 — Dead code: `ParamCollector` handles non-existent `DistinctOn` relation kind**
  `ParamCollector.mjs:44-47` — no `DistinctOn` relation node exists.
  **Fix:** Remove the dead branch.

- [ ] **m11 — Dead code: `ParamCollector` handles `Cast`/`CaseWhen` not in schema**
...
- [ ] **n1 — Pointless `catch (e) { throw e; }` in `cert-verify.mjs:54-56`**
  **Fix:** Remove the try/catch or add error enrichment.

- [ ] **n2 — Bats tests lack `WESLEY_REPO_ROOT` fallback**
...
- [ ] **n6 — Redundant `export` + `env` prefix in `qir-envelope-schema.bats:5`**
  **Fix:** Use one or the other.

- [ ] **n7 — Redundant `"additionalProperties": true` in `plan-report.schema.json:57`**
